...
# Test user login when the email is not verified
import pytest

async def test_login_unverified_user(db_session, unverified_user):
    # user exists but email_verified=False → should raise
    with pytest.raises(ValueError):
